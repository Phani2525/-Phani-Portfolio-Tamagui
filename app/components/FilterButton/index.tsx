...
export const FilterButton: React.FC<FilterButtonProps> = ({ onFiltersChange }) => {
  ...
  const [selectedFilters, setSelectedFilters] = useState<string[]>(filterOptions.map((option) => option.value));
    ...
    <>
      {filterOptions.map((option) => (
        <XStack animation="lazy" enterStyle={{ y: 50 }} space key={option.value}>
          <Checkbox checked={selectedFilters.includes(option.value)} onClick={() => toggleFilter(option.value)}>
            <Checkbox.Indicator>
              <Check />
            </Checkbox.Indicator>
          </Checkbox>
